...
│   │       ├── email/email.service.ts # Resend SDK wrapper
│   │       ├── modules/
...
│           │       ├── contacts/      # Contact list + enroll in sequence
...
33 Vitest test cases covering: ICP scoring engine, reply intent classification, RBAC middleware, sequence enrollment logic.
...
**Phase 2**
- [ ] Gmail / Outlook OAuth for real sending (currently mocked via Resend)
- [ ] Email open/click tracking via pixel + webhook
...
│   │       │   ├── index.ts           # 数据库连接 + 统一导出
│   │       │   └── migrate.ts         # 迁移执行脚本
│   │       ├── email/email.service.ts # Resend SDK 封装（EMAIL_ENABLED=false 时仅打印日志）
│   │       ├── modules/
...
共 33 个 Vitest 测试用例，覆盖：ICP 评分引擎、回复意图分类（Mock 模式）、RBAC 中间件、Sequence 加入逻辑。
...
**Phase 2**
- [ ] Gmail / Outlook OAuth 接入，实现真实收发件
- [ ] 邮件打开 / 点击追踪（Tracking Pixel + Webhook）
- [ ] Salesforce CRM 集成
